...
# Match MLPCpp RandomWeights() which uses uniform [-1.0, 1.0] for BOTH weights and biases
...
# ============================================================================
# 4. Physics-informed training loop with Gradient Annealing
# ============================================================================
...
# Match MLPCpp: physics_batch_size = 32
...
# Annealer Configuration (Matching MLPCpp)
alpha = 0.9
lambda_val = 1.0
lambda_min = 1e-4
lambda_max = 1e20

def get_grad_stats(grads):
    """Flattens gradients and computes max_abs and mean_abs."""
    flat = tf.concat([tf.reshape(g, [-1]) for g in grads if g is not None], axis=0)
    max_abs = tf.reduce_max(tf.abs(flat))
    mean_abs = tf.reduce_mean(tf.abs(flat))
...
for epoch in range(EPOCHS):
    ...
    for b in range(steps_per_epoch):
        ...
        # Tape for Physics Loss (L_r)
        with tf.GradientTape() as tape_phys:
            phys_loss = physics_residual_loss(model, x_coll_batch)
        g_phys = tape_phys.gradient(phys_loss, model.trainable_variables)
        g_phys = [g if g is not None else tf.zeros_like(v) for g, v in zip(g_phys, model.trainable_variables)]

        # Tape for Data Loss (L_i)
        with tf.GradientTape() as tape_data:
        ...
        g_ref = tape_data.gradient(data_loss, model.trainable_variables)
        g_ref = [g if g is not None else tf.zeros_like(v) for g, v in zip(g_ref, model.trainable_variables)]

        # --- Gradient Annealing Logic ---
        max_abs_phys, _ = get_grad_stats(g_phys)
        _, mean_abs_ref = get_grad_stats(g_ref)
        
        mean_abs_ref_val = float(mean_abs_ref)
        if mean_abs_ref_val < 1e-15:
            lambda_hat = lambda_max
        else:
            lambda_hat = float(max_abs_phys) / mean_abs_ref_val
            
        lambda_val = (1.0 - alpha) * lambda_val + alpha * lambda_hat
        lambda_val = np.clip(lambda_val, lambda_min, lambda_max)

        # Composite Gradient: L_r is base, lambda * L_i is weighted
        g_total = [gp + lambda_val * gr for gp, gr in zip(g_phys, g_ref)]
        opt.apply_gradients(zip(g_total, model.trainable_variables))

        total_loss = data_loss + lambda_val * phys_loss
    ...
    tf_lambda_hist.append(lambda_val)

    if epoch % 10 == 0 or epoch == EPOCHS - 1:
        print(f"{epoch + 1:<8}{epoch_data_loss:<16.4e}{epoch_phys_loss:<16.4e}"
              f"{lambda_val:<12.4f}{epoch_total_loss:<16.4e}")
        
# ============================================================================
# 5. Evaluate and Print Final Loss
# ============================================================================
...
print(f"Final total loss:                 {mse_raw + lambda_val * phys_final:.10e}")
print(f"======================================\n")

# Save TF history to CSV for completeness
with open("tf_pinn_training_history.csv", "w") as f:
    f.write("epoch,loss_data,loss_phys,lambda,loss_total\n")
    for i in range(EPOCHS):
        f.write(f"{tf_epochs[i]},{tf_data_loss_hist[i]},{tf_phys_loss_hist[i]},{tf_lambda_hist[i]},{tf_total_loss_hist[i]}\n")
...
try:
    ...
    # Plot 4: Lambda
    axs[1, 1].semilogy(tf_epochs, tf_lambda_hist, 'r-', label='TF Lambda', linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('results/pinn_annealed_comparison.png', dpi=300)
    print("Plot saved to pinn_annealed_comparison.png")
    plt.show()
    
except Exception as e:
    print(f"Could not read {cpp_file} or plot. Run the MLPCpp test case first. Error: {e}")